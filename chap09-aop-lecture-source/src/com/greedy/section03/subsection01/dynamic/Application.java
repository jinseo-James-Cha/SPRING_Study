...
public class Application {
	...
	public static void main(String[] args) {
		 ...
		 * 디자인 패턴의 프록시 패턴과는 조금 다른 개념이긴하다.
		 * 
		 * 실제 응용 프로그램을 만들 시, 프록시는 직접 기능을 구현할 일은 없다.
		 * 프록시는 대상의 요청을 가로채 기능을 확장시킬 목적으로 사용하여 Reflection기능을 이용 
		 * */
		
		Student student = new GreedyStudent();
		Handler handler = new Handler(student);
		student.study(16);// study method 실행 
		/* 프록시 만들기 */
		// 전달인자 1. 클래스로더 2. 프록시를 만들 클래스 메타정보(인터페이스만가능), 프록시 동작할 때 작용될 핸들
		...
		proxy.study(16);// -> study(16)의 메소드는 읽혀지지 않고
						// -> Handler의 invoke메소드만 사용되었다.
